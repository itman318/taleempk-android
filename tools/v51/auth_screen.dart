...
class _AuthScreenState extends State<AuthScreen> {
  ...
  Future<void> _next() async {
    ...
    try {
      if (register) {
        final result = await app.api.register(role: role, name: name.text.trim(),
        ...
        setState(() { register = false; step = 0; identifier.text = email.text.trim();
          confirm.clear(); notice = result.message; failed = false; });
        if (result.needsEmailVerification) {
          await _verifyEmail(app, identifier.text, password.text, needsReview: result.needsReview);
        }
        password.clear();
      } else {
        final result = await app.login(identifier.text.trim(), password.text);
        if (mounted && result.needsEmailVerification) {
          await _verifyEmail(app, identifier.text.trim(), password.text, address: result.email);
        }
      ...
      if (mounted && !register && e.status == 403 && e.message.toLowerCase().contains('verify your email')) {
        await _verifyEmail(app, identifier.text.trim(), password.text);
      } else if (mounted) { setState(() { notice = e.message; failed = true; }); }
    } catch (_) {
  ...
  Future<void> _verifyEmail(AppState app, String login, String secret, {String? address, bool needsReview = false}) async {
    final verified = await showModalBottomSheet<bool>(context: context, isScrollControlled: true,
      isDismissible: false, enableDrag: false, useSafeArea: true,
      builder: (_) => EmailVerificationSheet(api: app.api, identifier: login, password: secret,
        address: address, needsReview: needsReview));
    if (!mounted) return;
    setState(() {
      notice = verified == true
        ? (needsReview ? 'Email verified. Your account is waiting for admin approval.' : 'Email verified. Sign in to continue.')
        : 'Your account is created. Sign in to finish email verification.';
      failed = false;
    });
  }
